...
def charone():
    for a in box1.splitlines():
        ...
        cv2.putText(img1, a[0], (x, h1Img - y - 25), cv2.FONT_HERSHEY_PLAIN, 1.5, (0,
0, 255), 2)
...
def chartwo():
    for a in box2.splitlines():
        ...
        # Display detected letter under each bounding box
        cv2.putText(img2, a[0], (x, h2Img - y - 25), cv2.FONT_HERSHEY_PLAIN, 1, (0,
128, 0), 1)
    # Output the bounding box with the image
...
def charthree():
    for a in box3.splitlines():
        ...
        # Display detected letter under each bounding box
        cv2.putText(img3, a[0], (x, h3Img - y - 50), cv2.FONT_HERSHEY_PLAIN, 2, (255,
0, 0), 1)
    # Output the bounding box with the image
    ...
    for z, a in enumerate(data1.splitlines()):
        ...
        if z != 0:
            ...
            if len(a) == 12:
                ...
                cv2.putText(img1, a[11], (x - 15, y), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0,
255), 1)
    # Output the bounding box with the image
    cv2.imshow('Image output', img1)
    cv2.waitKey(0)
    ...
    for z, a in enumerate(data2.splitlines()):
        ...
        if z != 0:
            ...
            if len(a) == 12:
                ...
                # Display detected word under each bounding box
                cv2.putText(img2, a[11], (x - 15, y), cv2.FONT_HERSHEY_PLAIN, 2, (0,
255, 0), 1)
    # Output the bounding box with the image
    cv2.imshow('Image output', img2)
    cv2.waitKey(0)
    ...
    for z, a in enumerate(data3.splitlines()):
        ...
        if z != 0:
            ...
            if len(a) == 12:
                ...
                # Display detected word under each bounding box
                cv2.putText(img3, a[11], (x - 15, y), cv2.FONT_HERSHEY_PLAIN, 2, (255,
0, 0), 1)
    # Output the bounding box with the image
    cv2.imshow('Image output', img3)
    cv2.waitKey(0)
        ...
        for z, a in enumerate(data4.splitlines()):
            ...
            if z != 0:
                ...
                if len(a) == 12:
                    ...
                    # Display detected word under each bounding box
                    cv2.putText(frames, a[11], (x - 15, y), cv2.FONT_HERSHEY_PLAIN, 2,
(0, 0, 255), 1)
        # Output the bounding box with the image
    ...
    for z, a in enumerate(data4.splitlines()):
        ...
        if z != 0:
            ...
            if len(a) == 12:
                ...
                cv2.rectangle(frames, (x, y), (x + w, y + h), (0, 0, 255), 2)
                # Display detected word under each bounding box
                cv2.putText(frames, a[11], (x - 15, y), cv2.FONT_HERSHEY_PLAIN, 2, (0,
0, 255), 1)
    # Output the bounding box with the image
